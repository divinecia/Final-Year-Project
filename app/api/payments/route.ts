  ...
  try {
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        completedAmount: completedPayments.reduce((sum: number, p: any) => sum + (p.amount || 0), 0),
      },
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (validatedData.method === 'mobile_money') {
      ...
      processResult = await processMobileMoneyPayment(validatedData, transactionId);
    } else if (validatedData.method === 'card') {
      // Integrate with card payment processor
      processResult = await processCardPayment(validatedData, transactionId);
    } else if (validatedData.method === 'bank_transfer') {
      // Handle bank transfer
      processResult = await processBankTransfer(validatedData, transactionId);
    }
...
async function processMobileMoneyPayment(paymentData: any, transactionId: string) {
  try {
...
async function processCardPayment(paymentData: any, transactionId: string) {
  try {
    // This would integrate with card payment processor
    // For now, simulate successful processing
    return {
      success: true,
      message: 'Card payment initiated successfully',
      paymentUrl: `/payments/card/${transactionId}`
    };
  } catch (error) {
    return {
      success: false,
...
async function processBankTransfer(paymentData: any, transactionId: string) {
  try {
